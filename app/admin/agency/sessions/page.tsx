...
export default async function AgencySessionsPage({
  searchParams,
}: {
  ...
  const res = await adminApiGet("/api/admin/agency/sessions");
  const sessions = res.sessions || [];

  const filteredSessions = sessions.filter((s: any) => {
    const sPhone = String(s.phone || "");
    const sAgent = String(s.agent_name || "").toLowerCase();
    const sStatus = String(s.status || "").toLowerCase();

    return (
      (!phone || sPhone.includes(phone)) &&
      (!agent || sAgent.includes(agent)) &&
    ...
    <div className="space-y-5">
      <div className="rounded-t-2xl px-6 py-5 text-white shadow">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Monitor active agent sessions, expiry, device access, and revoke sessions.
        </p>
      </div>

      <form
        action="/admin/agency/sessions"
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 flex items-center justify-between">
          <Link
            href={`/admin/agency/sessions?${prevQuery.toString()}`}
          ...
          <Link
            href={`/admin/agency/sessions?${nextQuery.toString()}`}
